...
//            var spanBuilder = tracer.buildSpan(msg.action.asString)
//
//            // if cause is not None set the parent correctly
//            //todo get SpanContext from cause somehow
//            spanBuilder = spanBuilder.asChildOf()
//
...
//    }
    ...
    def run(msg: ActivationMessage, args: JsObject, timeout: FiniteDuration)(implicit system: ActorSystem, transid: TransactionId): RunResult = {
        ...
        logging.warn(this, s" cause              -----------------------:            ${msg.cause}")
